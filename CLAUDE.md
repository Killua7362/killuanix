...
- **`modules/common/programs/`** — individual program configs organized by category, each with its own `CLAUDE.md`:
  ...
  - [`cloud/azure-bastion/CLAUDE.md`](modules/common/programs/cloud/azure-bastion/CLAUDE.md) — `azure-cli` + `bastion-ssh` wrapper for DigitalAviations STG access via Azure Bastion. NixOS-only (chrollo+killua); username from sops `azure/bastion_username`. Currently a skeleton — real bastion/RG/VM resolution must be pasted from the DA runbook.
