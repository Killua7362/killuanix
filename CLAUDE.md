...
- **Trakt + TMDB media widgets** (`modules/containers/glance.nix` Home page) — "recently watched", trending movies + shows, and upcoming movie/TV calendars are driven by Trakt with TMDB poster enrichment. Trending widgets need only `trakt_api_key` (the OAuth app's Client ID from https://trakt.tv/oauth/applications) and `tmdb_api_key` (themoviedb.org). The personal calendars + history widgets additionally need an OAuth Bearer token: register the Trakt app with redirect URI `urn:ietf:wg:oauth:2.0:oob`, then run `TRAKT_CLIENT_ID=... TRAKT_CLIENT_SECRET=... trakt-auth.sh` (device flow — opens trakt.tv/activate, polls for approval, prints `access_token`). Paste the access token into sops as `trakt_access_token`, plus `trakt_username` (plaintext). Access tokens expire after ~3 months — re-run the script and rotate. No automated refresh service.
- **Matrix stack** (`modules/containers/matrix/`) — **DISABLED**: the `./matrix` import (plus its 6 quadlet volumes and the `matrix-net` network) is commented out in `modules/containers/default.nix`, so none of these containers build or run, and the stack no longer appears in the Glance service-bridge catalogue (`service-bridge/services.nix`). The module files are kept intact — re-enable by uncommenting the import + volumes + network in `modules/containers/default.nix` (sops secret declarations in `sops-system.nix` were left in place). The rest of this entry applies once re-enabled: Synapse + four mautrix bridges (Telegram / WhatsApp / Instagram / Messenger) + Element Web. `server_name = matrix.killua.local`, federation off, only `127.0.0.1:8008` (Synapse) and `127.0.0.1:8009` (Element Web) reach the host. All 7 containers ship with `autoStart = false` — start manually with `sudo systemctl start synapse element-web mautrix-telegram mautrix-whatsapp mautrix-meta-instagram mautrix-meta-messenger` (synapse cascades postgres + overrides + bridge-config oneshots via Requires). Before first `nixos-rebuild switch` you must populate ~20 keys in `secrets/personal.yaml` (per-bridge `as_token`/`hs_token`/`pickle_key`/`db_password`, plus Telegram `api_id`/`api_hash` from https://my.telegram.org). After it's up: `sudo podman exec -it synapse register_new_matrix_user -c /data/homeserver.yaml -c /etc/synapse/overrides.yaml -u akshay -a http://localhost:8008` → log in to Element Web at http://127.0.0.1:8009 → DM each `@*bot:matrix.killua.local` to authenticate the bridge. Full bootstrap walkthrough + pitfalls in `modules/containers/matrix/CLAUDE.md`.
- **Oracle SQLcl saved connections** (`modules/common/programs/cloud/azure-bastion/` + `modules/common/programs/dev/ai/oracle-sqlcl-mcp.nix`) — the `oracle-sqlcl` MCP server (Oracle SQLcl's built-in MCP server, `sqlcl -mcp`; `optional`, opt in per-project via `claude-kit.nix:mcp = [ "oracle-sqlcl" ]`) reads saved connections from `~/.dbtools/connections.json`. SQLcl stores that file under `$HOME`, not in nix store, so a fresh host has no connections. After first install on chrollo / killua, bring the bastion tunnel up (`BASTION_SSH_VIA_SOCKS=1 bastion sql`) and save a connection from the user shell — **the launcher is renamed to `sqlcl` in nixpkgs** to avoid clashing with GNU `parallel`'s `sql` binary, so do not run plain `sql`:
  ```
  sqlcl /nolog
  SQL> connect -save beastg1 -savepwd <user>/<pass>@127.0.0.1:1521/beastg1
  SQL> exit
  ```
  Repeat per service name you want (`beastg1`..`beastg6` for the 6 dev stages — Oracle picks the stage from the SERVICE_NAME, the tunnel is stage-agnostic). The MCP server then exposes each connection by name. The DataGrip / SQL Developer flows are unaffected (they own their own connection state under `~/.config/JetBrains/` and `~/.sqldeveloper/` respectively).
- **LibreOffice MCP extension** (`modules/common/mcp-servers.nix` `libreoffice` entry — `jwingnut/mcp-libre`) — the MCP registry entry only wires the stdio FastMCP **bridge** (`libreoffice_mcp_server.py`); it talks to an HTTP server (`:8765`) that lives **inside** LibreOffice itself, shipped as an `.oxt` extension that nix does not package. Without the extension installed, the bridge has nothing to call.
  - One-time setup per host (apply the autostart patch from this repo, build, install):
    ```
    git clone https://github.com/jwingnut/mcp-libre /tmp/mcp-libre
    cd /tmp/mcp-libre
    patch -p1 < ~/killuanix/modules/common/programs/dev/ai/patches/mcp-libre-extension-autostart.patch
    cd plugin && bash ./build.sh   # `bash` invocation because build.sh's shebang is `/bin/bash` (absent on NixOS)
    unopkg add ../build/libreoffice-mcp-extension-*.oxt
    ```
  - Day-to-day: launch soffice via **`soffice-mcp`** (HM module `modules/common/programs/dev/ai/libreoffice-mcp-launcher.nix`) instead of bare `soffice`. The wrapper opens soffice with a UNO `--accept=` socket, then forces LO to import the extension's python module via `createInstanceWithContext("org.mcp.libreoffice.MCPExtension")` — that triggers the autostart thread in the patched `registration.py`, which binds the MCP HTTP server on `127.0.0.1:8765`. Bare `soffice` does NOT load the python module at startup (LO only imports it on first dispatch query, which never reliably fires because upstream's `ProtocolHandler.xcu` routing is broken — LO's framework returns its built-in `ServiceHandler` stub for `service:org.mcp.libreoffice.MCPExtension*` URLs instead of the extension's `MCPProtocolHandler`).
  - Opt in per-project via `claude-kit.nix:mcp = [ "libreoffice" ];` then `claude-kit project sync`. Bridge operates on the **live** document model via UNO (multi-doc, real-time) — soffice must be running through `soffice-mcp` with at least one document open before claude can query.
- **Oracle Database 19c VM** (`modules/vms/oracle-19c-vm.nix`) — libvirt domain `oracle-19c` on chrollo, 8 GiB / 2 vCPU, qcow2 at `~/VMs/oracle-19c.qcow2`, virtio NIC on the default libvirt NAT (192.168.122.0/24). Migrated off VirtualBox: chrollo now has `vms.virtualbox.enable = false` and the previous `kvm_intel`/`kvm` blacklist removed so `/dev/kvm` is available. After a fresh switch + reboot (or `sudo modprobe kvm_intel`), run `oracle-vm-import.sh` once — it picks the newest `oracle-19c*.ova` in `~/Downloads` and `qemu-img convert`s the embedded VMDK into `~/VMs/oracle-19c.qcow2` (~30-50 GB sparse). Pass `--force` to replace an existing qcow2, or a path arg to pick a specific OVA. Start with `virsh -c qemu:///system start oracle-19c` (or virt-manager); discover the guest IP via `virsh -c qemu:///system net-dhcp-leases default`. The internal Oracle listener stays on `0.0.0.0:1521` inside the appliance, so `sqlplus system/<pw>@//<vm-ip>:1521/ORCLCDB` works directly from the host — no host port forwards. The old `~/VirtualBox VMs/oracle-19c-vagrant/` tree is left untouched as a fallback; delete once the migrated VM is healthy.
...
- **RTK (Rust Token Killer)** (`packages/rtk/` for the binary; `modules/common/programs/dev/ai/rtk.nix` for the Claude Code hook) — CLI proxy (`rtk`) that compresses verbose dev-command output (git/gh/ls/tree/test/psql/aws/json…) before it reaches an LLM, cutting token use 60-90%. Not in nixpkgs; built from the upstream git tag `v0.43.0` via `rustPlatform.buildRustPackage` with the vendored `packages/rtk/Cargo.lock` driving deps (no `cargoHash` to maintain — bump `version` + `src.hash` on upgrade, refresh `Cargo.lock` from the new tag). Binary is added to `commonPackages` (all hosts). **The auto-rewrite hook is fully declarative — no `rtk init -g` needed.** `rtk.nix` registers rtk's native single-binary hook processor (`rtk hook claude`) as a Claude Code `PreToolUse` hook (matcher `Bash`) through claude.nix's `local.extraHooks` side-channel; the pinned rtk binary reads the PreToolUse JSON on stdin and emits the command rewrite, so it can't drift from the installed version and needs no jq/script/state. Verify after a switch with `rtk --version` / `rtk gain` (token-savings stats), then restart Claude Code so the new settings.json loads.
- **Claude Code secret-dir isolation (Option B)** (`modules/nixos/claude-sudo.nix` + the bwrap block in `modules/common/programs/dev/ai/claude.nix`'s `overlayClaude` + `deny-secrets.ts` in `Notes/claude/hooks/`) — Claude runs as the `killua` user but two dirs are made **kernel-inaccessible** to it and every process it spawns: `/home/killua/killuanix/secrets` and `~/.config/sops/age`. Mechanism: every `claude` invocation (bare, ccmanager, launchers — all resolve to `overlayClaude`) re-execs under `bwrap --dev-bind / /` with an empty `--tmpfs` masking each secret dir (Linux-only; needs unprivileged user namespaces, on by default; fails **closed** if bwrap can't start). All *other* files stay reachable. Privilege model: `sudo ss <anything>` is passwordless (`claude-sudo.nix`, both hosts — `ss` is read-only, not a root-escape gadget, so it can't tear the namespace down); **every other** privileged command must go through `pkexec` → the `hyprpolkitagent` GUI dialog (password agent→polkitd, never on disk/in Claude; stdout returns normally). `nixos-rebuild` is deliberately **not** passwordless — that would be an unattended-root escape hatch that defeats the masking. Belt-and-suspenders: `permissions.deny` in `claude.nix` blocks the Read/Edit/Write tool surface, and `deny-secrets.ts` blocks the `pkexec`-elevated Bash bypass (runs as root outside the namespace) before the GUI dialog. **pkexec broker** (`modules/common/programs/dev/ai/pkexec-broker/`): because a `pkexec` launched *inside* the sandbox inherits the mask (empty secret dirs → `nixos-rebuild`/sops would break), a `systemd --user` broker in the graphical session runs the real setuid `pkexec` **outside** the namespace (secret dirs visible, GUI dialog via `hyprpolkitagent`, output streamed back); a `pkexec` client shim is injected onto Claude's PATH **inside the sandbox only** (via `claude.nix`'s `local.claudeExtraPath` → `overlayClaude`'s `bwrap --setenv PATH`), so `claude` can `pkexec nixos-rebuild …` (GUI-approve → runs unmasked → succeeds) while the user's own terminal keeps the real pkexec. A second shim name — **`nix_switch`** (same script, broker kind `nixswitch`) — makes `nix_switch both` work from the sandbox: the broker runs the allowlisted real `nix_switch` **as the user** (not root), forced `--no-nh`, with `NIX_SWITCH_ESCALATE=pkexec` so the home half applies unmasked and the system half's `pkexec nixos-rebuild` pops the GUI dialog (see the escalate branch in `DotFiles/scripts/personal/nix_switch`). Trade-off: no `nh` closure diff for Claude-run switches. So Claude *can* trigger a system switch, but only through a GUI dialog you review + approve — the age key is read legitimately by your vetted config at that moment (review the diff before approving; a malicious `activationScripts`/`systemd.services`/`sops.templates` line would run as root). Verify after switch: `sudo ss -tulpn` (no prompt), then in a `claude` session `ls ~/.config/sops/age` and `cat ~/killuanix/secrets/*` both show empty/blocked. To temporarily disable the mask, launch via a non-wrapped claude (not recommended). CLAUDE.md instruction to Claude: use `pkexec <cmd>` (or `systemctl` D-Bus) for privileged commands, never `sudo` (except `sudo ss`).
...
- **`modules/common/programs/`** — individual program configs organized by category, each with its own `CLAUDE.md`:
  ...
  - [`chat/CLAUDE.md`](modules/common/programs/chat/CLAUDE.md) — Ferdium messaging aggregator. NixOS-only (chrollo+killua), palette-driven `custom.css`. Services/credentials are GUI-managed; sops holds a recipe-list runbook only.
  - [`cloud/azure-bastion/CLAUDE.md`](modules/common/programs/cloud/azure-bastion/CLAUDE.md) — `azure-cli` + a single `bastion` dispatcher (`bastion ssh|sql|login|pg|cosmos`, scripts in `DotFiles/scripts/boeing/`) for DigitalAviations STG access via Azure Bastion. NixOS-only (chrollo+killua); distro-agnostic scripts read one env file (`$BASTION_ENV_FILE`, default `~/.config/azure-bastion.env`) rendered from sops `azure/*` via `sops.templates`.
  ...
  - [`dev/CLAUDE.md`](modules/common/programs/dev/CLAUDE.md) — Git, Lazygit → [`ai/CLAUDE.md`](modules/common/programs/dev/ai/CLAUDE.md) (Claude Code + bundled skills, OpenCode, ccmanager, ccr, ruflo, claude-flow, claude-kit, jupyter-env MCP)
  ...
  - [`terminal/CLAUDE.md`](modules/common/programs/terminal/CLAUDE.md) — WezTerm (primary terminal + native mux under Hyprland; replaced tmux + zellij), Ghostty + Kitty (fallback emulators)
...
- **`modules/containers/`** — quadlet / portainer container definitions (litellm, mcphub, searxng, portainer, glance, excalidraw, mermaid-live, karakeep, icloud-drive, cronicle, boeing, matrix (disabled — see below) → [`matrix/CLAUDE.md`](modules/containers/matrix/CLAUDE.md), freshrss → [`freshrss/CLAUDE.md`](modules/containers/freshrss/CLAUDE.md), cliphist-viewer → [`cliphist-viewer/CLAUDE.md`](modules/containers/cliphist-viewer/CLAUDE.md))
...
- `sdethings/`, `Notes/` — ad-hoc scratch and notes. (Repo scripts moved into the `DotFiles` submodule: `DotFiles/scripts/personal/` holds `nix_switch`, `trakt-auth.sh`, `wt`, `zj-proj`, `fetch-deps`, etc.; `DotFiles/scripts/boeing/` holds `oracle-vm-import.sh`, `gp-fastest-gateway.sh`, `azure-blob-{up,down}load.sh`, a single **`bastion` dispatcher** (`bastion ssh|sql|login|pg|cosmos`) fronting `bastion.d/bastion-*` + shared `bastion.d/_lib.sh` (moved out of `modules/common/programs/cloud/azure-bastion/`; distro-agnostic — secrets from one env file `$BASTION_ENV_FILE`, default `~/.config/azure-bastion.env`, rendered on nix via `sops.templates`), and the separate upstream `ubuntu-bastion/` tool; older shell utilities live in `DotFiles/scripts/v1/`. All three dirs are on `PATH` via the shell configs, so scripts are invoked by bare name.)
