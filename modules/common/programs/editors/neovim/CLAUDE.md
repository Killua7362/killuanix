...
| `java.lua` | nvim-jdtls | Java LSP with nixCats-aware paths for jdtls, lombok, debug adapter, and test bundles; which-key integration for extract/organize/test commands. Both **maven** and **eclipse** importers are enabled — jdtls disambiguates per-project from `pom.xml` vs `.project` markers, so a Maven Spring Boot worktree and an Eclipse-format ATG tree can coexist. Gradle stays off. jdtls is offered **two** compile runtimes and picks per-project by matching each project's required execution environment: the **default** is `$JAVA_HOME` (nix JDK 25 → `JavaSE-25`, name auto-derived from `<home>/release`), plus the legacy `~/Downloads/java/jdk1.8.0_291` registered non-default as `JavaSE-1.8` so the Java-8 Eclipse project `bdsi-commerce-code` still resolves after the global `JAVA_HOME` moved to JDK 25. Java 8's `JAVA_VERSION="1.8.0…"` is mapped to `JavaSE-1.8` (not `JavaSE-1`); 9+ map to a plain major. Falls back to `JavaSE-21` only if neither home is a valid JDK. jdtls itself runs under the nix `jdt-language-server` wrapper's own JDK — these entries are only the compile/classpath runtimes. |
