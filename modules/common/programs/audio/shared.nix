{
  ...
  options.audio.bluetooth.enableMsbc = lib.mkOption {
    type = lib.types.bool;
    default = true;
    description = ''
      Whether to enable mSBC (16 kHz wideband) for HFP/HSP headset calls.
      Disable on hosts where the native HFP backend can't open an mSBC SCO
      transport (observed on the Intel Lunar Lake BT controller — "failed
      to get HFP codec 2"), which leaves HFP unregistered. With this off,
      HFP falls back to CVSD (8 kHz narrowband) and the profile activates
      cleanly. Most controllers handle mSBC fine, so the default is true.
    '';
  };
  ...
  config = {
    ...
    # ╚══════════════════════════════════════════════════════════════════╝
    # NOTE: do NOT enable `services.blueman-applet` here. The blueman package
    # already ships its own user unit (share/systemd/user/blueman-applet.service)
    # with `ExecStart=`, and the HM module's drop-in adds a second `ExecStart=`
    # without first clearing it, which systemd refuses ("Service has more than
    # one ExecStart= setting"). The applet is launched by Hyprland's exec-once
    # instead (modules/common/programs/desktop/hyprland/execs.nix).
    ...
    xdg.configFile = {
      ...
      "wireplumber/wireplumber.conf.d/50-bluez-config.conf".text = ''
        monitor.bluez.properties = {
            ...
            bluez5.enable-msbc      = ${lib.boolToString config.audio.bluetooth.enableMsbc}
