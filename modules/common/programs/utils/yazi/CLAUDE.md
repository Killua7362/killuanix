...
| File | Description |
|---|---|
| `default.nix` | Nix module that enables Yazi, wires in zsh shell integration, imports `yazi.toml`/`keymap.toml`/`theme.toml` via `lib.importTOML`, and declares plugins/flavors. Uses the `yazi` flake input's package with `_7zz` overridden to `pkgs._7zz-rar` for RAR extraction support. |
