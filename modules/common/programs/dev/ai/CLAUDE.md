...
- **Claude Code skills**: No hashes are maintained. Two registration paths feed `~/.claude/skills/`:
  ...
  2. **Nix-store-pinned, always-on, cherry-picked from flake inputs** — `extraSkills` in `claude.nix` (e.g. `foo = "${inputs.bar}/.../foo"`). Currently holds the `gstack` umbrella + `office-hours` pair (the umbrella is needed because office-hours's SKILL.md references `~/.claude/skills/gstack/bin/...`). Anything added here is passed through `programs.claude-code.skills = extraSkills` and requires `scripts/nix_switch` to update.
...
- `extraSkills` in `claude.nix` — `gstack` (umbrella) + `office-hours` from `inputs.gstack`; `grill-me` + `handoff` from `inputs.mattpocock-skills` (self-contained, no umbrella needed).
