...
| `ccstatusline.nix` | Declarative Claude Code status line. Lazy-`npx ccstatusline@<pinned>` shim (same shape as `ruflo-cli.nix`) installs the `ccstatusline` binary; `xdg.configFile."ccstatusline/settings.json"` is rendered from a Nix attrset and lands as a read-only nix-store symlink (TUI saves silently fail — edit the `cfg` attrset and `nix_switch`). Wired into `programs.claude-code.settings.statusLine` in `claude.nix` via `command = "ccstatusline"` (PATH lookup, so `claude-launchers.nix` wrappers inherit it). Default layout is a single line: `model · thinking-effort · context-percentage · git-branch · current-working-dir`. Colours come from `config.theme.palette` via ccstatusline's `hex:RRGGBB` literal — requires `colorLevel = 3` (truecolor) to render; chalk's named ANSI colours are bypassed. Widget type strings come from upstream's `WIDGET_MANIFEST` (`src/utils/widget-manifest.ts`); top-level `Settings` schema lives at `src/types/Settings.ts`; `hex:`/`ansi256:` literals are accepted by `src/utils/colors.ts:getChalkColor`. |
