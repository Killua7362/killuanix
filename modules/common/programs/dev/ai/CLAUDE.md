...
- **Claude Code skills**: No hashes are maintained. Two registration paths feed `~/.claude/skills/`:
  ...
  2. **Nix-store-pinned, always-on, cherry-picked from flake inputs** — `extraSkills` in `claude.nix` (e.g. `foo = "${inputs.bar}/.../foo"`). Currently empty — was the home for `er-diagram-and-data-modeling`, now project/launcher-scoped. Anything added here is passed through `programs.claude-code.skills = extraSkills` and requires `scripts/nix_switch` to update.
...
- Every `claude` boot:
  - **No CLAUDE_CONFIG_DIR pre-set** → mktemp a per-PID dir under `${XDG_RUNTIME_DIR:-/tmp}/claude-session.XXXXXX`, mirror every entry of `~/.claude/` as symlinks *except* `settings.json` (copied in as a writable real file via `install -m 0644`) and the entries in `caveman_skip_symlink` (`.caveman-active`, `.caveman-statusline-suffix`), then `$HOME/.claude.json` (auth + onboarding state — sibling of `~/.claude/`, **not** inside it; without this every launch lands in re-auth) is symlinked into the dir, `CLAUDE_CONFIG_DIR=<state_dir>` exported, and an EXIT/INT/TERM/HUP trap installed that copies `.credentials.json` back to `~/.claude/` (only if it's now a real file — Claude's OAuth-refresh path does a `tmp + rename` atomic write that replaces the inbound symlink with a regular file, so without this copy-back the refreshed token is wiped with the overlay and the next launch reads the stale expired one → forced re-login every session) and then `rm -rf`'s the dir. The caveman files are *neither symlinked nor pre-copied* — caveman-activate.js creates `.caveman-active` fresh inside the overlay on SessionStart, and the Stop hook (see "Stop hook — per-session statusline savings badge" under "## Always-on plugins") writes `.caveman-statusline-suffix` after each assistant turn. Per-session isolation depends on these files staying inside the overlay — symlinking the suffix through would let every concurrent claude process's Stop hook write through to one shared real file, and every session's badge would render the same number. Symlinking `.caveman-active` would additionally block caveman-activate.js's `safeWriteFlag` (caveman-config.js:122-141 — defence against attacker pointing the flag at `~/.ssh/id_rsa`), and the statusline would never show any `[CAVEMAN]` badge.
...
- `extraSkills` in `claude.nix` (currently empty — `er-diagram-and-data-modeling` moved to per-launcher / per-project opt-in via the `vibekit` flake input).
...
**Stop hook — per-session statusline savings badge**: `programs.claude-code.settings.hooks.Stop` (set in `claude.nix`) writes a *per-session* `⛏ <Nk>` suffix into the overlay's `.caveman-statusline-suffix` after every assistant turn. Each concurrent claude session's badge therefore shows **only that session's saved tokens** — lifetime / 5h-block aggregation lives separately in the `caveman stats` shell wrapper (see entry below).
