...
1. The best **model** — one of: `claude-opus-4-8`, `claude-opus-4-6`, `claude-sonnet-4-6`
...
The goal is to help the user stop reflexively running everything at Opus 4.8 + max, which is the single biggest source of wasted tokens.
...
Use the literal model IDs (`claude-opus-4-8`, `claude-opus-4-6`, `claude-sonnet-4-6`) — not friendly names like "Opus 4.8".
...
### 2. Quick coding / single-file work → claude-sonnet-4-6 + medium, OR claude-opus-4-8 + high
...
→ Default `claude-sonnet-4-6` + `medium`. Bump to `claude-opus-4-8` + `high` only if the algorithm is genuinely novel or the bug spans tricky concurrency/state. Plan mode: `No`.

### 3. Multi-file / multi-step coding → claude-opus-4-8 + xhigh, plan mode Yes
...
→ `claude-opus-4-8` + `xhigh`, plan mode `Yes`. Plan phase: `claude-opus-4-8 + xhigh`. Execution phase: `claude-opus-4-8 + high`.

### 4. Genuinely hard / correctness-critical → claude-opus-4-8 + max
...
→ `claude-opus-4-8` + `max`, plan mode `Yes`. Plan phase: `claude-opus-4-8 + xhigh`. Execution phase: `claude-opus-4-8 + max`.

### 5. Long agentic / autonomous run → claude-opus-4-8 + xhigh

If the user is kicking off something they intend to leave running (multi-hour, "build me X", auto mode, async agent):

→ `claude-opus-4-8` + `xhigh`, plan mode `Yes`. Don't recommend `max` here — `max` overthinks on long runs and burns tokens unpredictably.
...
**Why:** Opus 4.8 is literal and underperforms on vague prompts — model choice can't fix that. Add: intent, constraints, acceptance criteria, file paths. Then re-run.
```
...
Otherwise default to `claude-opus-4-8` — at the same effort level it generally outperforms 4.6 with comparable or fewer tokens on coding tasks.
...
|---|---|---|---|
| Plan | `claude-opus-4-8` | `xhigh` | Plan quality compounds; spend tokens here |
| Execute | `claude-opus-4-8` | `high` | Working from a clear plan, full xhigh is overkill |
...
- Don't recommend `claude-opus-4-6` unless the user has a specific reason. `claude-opus-4-8` at the same effort is usually better and sometimes cheaper.
