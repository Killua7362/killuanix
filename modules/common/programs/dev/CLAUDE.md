...
- **Key config choices**:
  ...
  - `statusHooks` (Linux only, `lib.optionalAttrs pkgs.stdenv.isLinux`) fire `notify-send` on `idle` / `waiting_input` (critical urgency). macnix skips this block entirely.
...
- **Multi-project root**: `home.sessionVariables.CCMANAGER_MULTI_PROJECT_ROOT = "$HOME/ccmanager-projects"`. The farm is built by `home.activation.ccmanagerProjects` from the `ccmanagerProjects` attrset at the top of `ccmanager.nix` — add a project by appending `<name> = "<abspath>";`. Symlinks are created unconditionally (no `.git` probing), which is what lets bare-repo worktree layouts (e.g. `~/Documents/Boeing/azure/backend`) work. The activation block only deletes **symlinks** from the farm, so any real files the user drops in are preserved.
